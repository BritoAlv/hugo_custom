# References Example

This folder is part of this repository and is published as the
`examples/references/` section of the site. It demonstrates references
between files. From here, links are flat paths relative to this folder:

- [notes/overview.md](notes/overview.md)
- [notes/setup.md](notes/setup.md)
- [notebooks/analysis.ipynb](notebooks/analysis.ipynb)
- [code/main.py](code/main.py)
- [assets/diagram.svg](assets/diagram.svg)
- [assets/sample.csv](assets/sample.csv)